...
import AmazonSvg from '../company_svg/amazon.webp';
...
import AppleSvg from '../company_svg/apple.png';
...
const EmblaCarousel = () => {
    const slides = [[AmazonSvg, false], [PinterestSvg, false], [MetaSvg, false], [NetflixSvg, true], [AppleSvg, false], [GoogleSvg, false], [UberSvg, true]]
        ...
        <section className="w-full">
            <div className="overflow-hidden" ref={emblaRef}>
                <div className="flex touch-pan-y touch-pinch-zoom ml-[-1rem]">
                    {slides.map(([svg, enlarge], index) => (
                        <div className="translate-x-0 flex-[0_0_30%] min-w-0 pl-4" key={index}>
                            <div className="inset-shadow rounded-3xl text-4xl font-semibold flex items-center justify-center h-[9rem] select-none">
                                <Image src={svg} alt={svg} className={enlarge ? "w-[10rem]" : "w-[5rem]"} />
                            </div>
                        </div>
                    ))}
                </div>
